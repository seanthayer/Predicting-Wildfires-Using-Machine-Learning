...
def parseEntry(entry):
  ...
  X = entry["geometry"]["x"]
  Y = entry["geometry"]["y"]
  foundLng = entry["attributes"]["InitialLongitude"] if entry["attributes"]["InitialLongitude"] else ''
  foundLat = entry["attributes"]["InitialLatitude"] if entry["attributes"]["InitialLatitude"] else ''

  return ",".join([county, str(fireDate.year), str(fireDate.month), str(fireDate.day), str(X), str(Y), str(foundLng), str(foundLat)]) + '\n'
...
request = requests.get(url = NIFC_esri_feature_service_query_base.format(query_result_offset // query_result_batch))
NIFC_dataset_partition = request.json()

print("[INFO] Dataset processing begin")

while len(NIFC_dataset_partition["features"]):
  print("[INFO] Processing dataset batch {} (rows = {})".format(query_result_offset // query_result_batch, len(NIFC_dataset_partition["features"])))
  ...
  query_result_offset += query_result_batch
...
fileIO.write("County,Year,Month,Day,X,Y,foundLng,foundLat\n")
